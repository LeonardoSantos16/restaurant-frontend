...
Define the structure and behavior of the about page (`/sobre`) — a static,
editorial page that communicates the restaurant's identity: an atmosphere
spotlight hero with a positioning statement as its heading, a short passage of
philosophy prose (no labeled "story" header), a practical information block
(hours, address, contact) as a definition list with numeric values in mono, and
a single discreet reservation call to action. It reuses the existing visual
language (spotlight, featured-section pattern) and introduces no new components.

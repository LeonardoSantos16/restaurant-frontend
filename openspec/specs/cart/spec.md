...
Define the behavior of the shopping cart: the `/carrinho` page that lists
selected dishes with editable quantities and a total, and the cart drawer
that opens automatically to give quick, in-place feedback whenever an item is
added to the cart from any other page.
